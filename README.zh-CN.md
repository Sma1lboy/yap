...
手写配置：Yap 会在 config.json 旁边放一份 `config.schema.json`（启动时写入），并在它写出的文件顶部加上 `"$schema": "./config.schema.json"`。支持 JSON Schema 的编辑器（VS Code 默认就支持）会据此补全字段名和可选值，并在你输入时标出写错的地方；旧文件自己加上这一行即可。
